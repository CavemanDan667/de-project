...
class TestUploadObject:
    ...
    def test_upload_object_returns_correct_object_name(
        self, s3, caplog
    ):  # noqa
        s3.create_bucket(
    ...
    def test_write_data_converts_dictionary_into_csv_and_uploads_to_s3_bucket(
        self, s3, caplog
    ):  # noqa
        s3.create_bucket(
            Bucket="de-project-ingestion-bucket",
        ...
        response_body = (
            s3.get_object(
                Bucket="de-project-ingestion-bucket", Key="currency/0001.csv"
            )["Body"]
            .read()
            .decode("utf-8")
        )  # noqa
    ...
    def test_key_error_raised_when_passed_dictionary_not_containing_rows(
        self, caplog
    ):  # noqa
        with pytest.raises(KeyError):
            data = {"Headers": ["column_1", "column_2",
                                "column_3", "last_updated"]}
            write_data_to_csv("00001", "test_table", data)
        assert "KeyError: key 'Rows' not found" in caplog.text

    def test_key_error_raised_when_passed_dictionary_without_headers(
        self, caplog
    ):
        with pytest.raises(KeyError):
            data = {
    ...
    def test_file_not_found_error_raises_when_passed_file_that_does_not_exist(
        self, caplog
    ):  # noqa
